...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['verify'])) {
    ...
    if ($entered_otp == $_SESSION['otp']) {
        ...
        $check_sql = "SELECT * FROM borrowers WHERE borrower_email = '$email'";
        $result = $conn->query($check_sql);

        if ($result->num_rows > 0) {
            // If email already exists, show pop-up message
            echo "<script>
                    alert('This email is already registered. Please login or use a different email.');
                    window.location.href = 'student_signup.html';  // Redirect to registration page
                  </script>";
        } else {
            // Insert new user into the database
            $sql = "INSERT INTO borrowers (borrower_name, borrower_email, password) VALUES ('$name', '$email', '$password')";
